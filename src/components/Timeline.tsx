...
const Timeline: React.FC = () => {
  const timelineData = useSelector(
    (state: RootState) => state.timeline.timelineData,
  );

  // ✅ FIXED: Smooth staggered animations - No blinking
          ...
          >
            {timelineData.map((event, index) => (
              ...
              >
                ...
                {/* Content Card */}
                <motion.div
                  className={`lg:flex-1 lg:w-full max-w-lg lg:max-w-xl p-4 lg:p-8 lg:py-10 bg-white/8 backdrop-blur-2xl border border-white/15 rounded-3xl shadow-2xl hover:shadow-2xl hover:shadow-cyan-500/25 hover:border-cyan-400/40 transition-all duration-700 ml-16 lg:ml-12 ${
                ...
                >
                  <h3 className="text-lg md:text-2xl lg:text-3xl font-bold text-white mb-5 leading-tight tracking-tight group-hover:text-cyan-300 transition-colors duration-500">
                    {event.title}
                  </h3>
